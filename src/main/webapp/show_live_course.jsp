...
<script type="text/javascript">
    ...
    var size = 15;
    var all_url = url+"/admin/showLiveCourse.do?page="+page+"&size="+size;
    //--------------------------------------------------------------------
    // 修改虚拟预约用户数(添加输入框)
    //判断是否有输入框
        ...
        $.ajax({
            ...
            success:function (result) {
                ...
                for(var i = 0; i < data.length; i++){
                    $("#info").append('<tr>'+
                        '<td>'+data[i]['id']+'</td>'+
                        '<td>'+data[i]['periods']+'</td>'+
                        '<td>'+data[i]['st']+'</td>'+
                        '<td>'+data[i]['et']+'</td>'+
                        '<td>'+data[i]['enrollment']+'</td>'+
                        '<td><button style="margin-left: 5px;" onclick="single('+"'"+data[i]['id']+"'"+')">查看</button><button style="margin-left: 5px;" onclick="del('+"'"+data[i]['id']+"'"+')">删除</button>(若删除，参加该期的的所有用户记录也会删除)</td>'+
                        '</tr>');
                }
    ...
    <table cellpadding="9" width="87%" border="1" cellspacing="0" id="info">
        <tr>
            ...
            <td style="border-left: 0;">
                <button style="float: right"><a href="add_live_course.jsp">新建</a></button>
            </td>
        </tr>
        <tr>
            <td>序号</td>
...
<script>
    function single(id) {
        window.location.href = "show_live_course_userInfo.jsp?id="+id;
    }
    function del(id) {
        if (confirm("你确定要删除？")){
            if (confirm("若删除，参加该期的的所有用户记录也会删除，你确定要删除？")){
                $.ajax({
                    url:url+"/admin/deleteLiveCourse.do",
                    type:'POST',
                    data:{
                        id:id
                    },
