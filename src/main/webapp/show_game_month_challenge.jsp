...
<script type="text/javascript">
    ...
    var size = 15;
    var all_url = url+"/admin/showGameMonthChallenge.do?page="+page+"&size="+size+"&type="+type;
        ...
        $.ajax({
            ...
            success:function (result) {
                ...
                if (page == 1){
                    ...
                    $("#page").append('<td><a href="'+url+'/show_game_month_challenge.jsp?page=1&size='+size+'">第一页</a></td>');
                }
                var ff = 0;
                var f = 0;
                if (page > 4){
                ...
                for(var z = f; z < page_no; z++){
                    ...
                    if (no == page){
                        ...
                        $("#page").append('<td><a href="'+url+'/show_game_month_challenge.jsp?page='+no+'&size='+size+'">'+no+'</a></td>');
                    }
                    if (ff == 8)break;
                    ff++;
                }
                ...
                if (page == page_no){
                    ...
                    $("#page").append('<td><a href="'+url+'/show_game_month_challenge.jsp?page='+page_no+'&size='+size+'">最后一页</a></td>');
                }
                for(var i = 0; i < data.length; i++){
                    var string1;
                    if (data[i]['is_deliver']=='0'){
                        string1 = '未确认'
                    }else {
                        string1 = '已确认';
                    }
                    $("#daily_data").append('<tr>'+
                        '<td>'+data[i]['time']+'</td>'+
                        '<td>'+string1+'</td>'+
                        '<td><button style="margin-left: 5px;" onclick="sent('+"'"+data[i]['id']+"'"+')">发放</button><button style="margin-left: 5px;" onclick="check('+"'"+data[i]['id']+"'"+')">查看</button></td>'+
                        '</tr>');
                }
...
<script>
    function sent(id) {
        if (confirm("你确定要结束挑战？")){
            $.ajax({
                url:url+"/admin/gameMonthChallengeCommit.do",
                type:'POST',
                data:{
                    id:id
                },
    ...
    function check(id) {
        window.location.href = "single_game_challenge.jsp?id="+id;
    }
</script>
</html>
