...
<script type="text/javascript">
    ...
    var size = 15;
    var all_url = url+"/admin/show_virtual_user_game.do?page="+page+"&size="+size+"&type="+type;
        ...
        $.ajax({
            ...
            success:function (result) {
                ...
                if (page == 1){
                    ...
                    $("#page").append('<td><a href="'+url+'/show_virtual_user_game.jsp?page=1&size='+size+'">第一页</a></td>');
                }
                var ff = 0;
                var f = 0;
                if (page > 4){
                ...
                for(var z = f; z < page_no; z++){
                    ...
                    if (no == page){
                        ...
                        $("#page").append('<td><a href="'+url+'/show_virtual_user_game.jsp?page='+no+'&size='+size+'">'+no+'</a></td>');
                    }
                    if (ff == 8)break;
                    ff++;
                }
                ...
                if (page == page_no){
                    ...
                    $("#page").append('<td><a href="'+url+'/show_virtual_user_game.jsp?page='+page_no+'&size='+size+'">最后一页</a></td>');
                }
                ...
                for(var i = 0; i < data.length; i++){
                    ...
                    $("#author_info").append('<tr>'+
                        ...
                        '<td style="width: 4%;">'+data[i]['expRank']+'</td>'+
                        '<td style="width: 6%;">'+data[i]['challengeRank']+'</td>'+
                        '<td style="width: 6%;">'+data[i]['game_present_month_exp']+'</td>'+
                        '<td style="width: 6%;">'+data[i]['lv']+'</td>'+
                        '<td style="width: 6%;">'+data[i]['game_exp']+'</td>'+
                        '<td style="width: 6%;"><input placeholder="正数或负数" type="text" id="add_number' + data[i]['id'] +'"><button style="margin-left: 5px;" onclick="user_add_day('+"'"+data[i]['id']+"'" + ')">添加</button><button style="margin-left: 5px;" onclick="del('+"'"+data[i]['id']+"'"+')">删除</button></td>'+
                        '</tr>');
                }
    ...
    <table cellpadding="9" width="87%" border="1" cellspacing="0" id="author_info">
        <tr>
            ...
            <td style="border-left: 0;">
                <button style="float: right"><a href="add_virtual_user_game.jsp">新建</a></button>
            </td>
        </tr>
        <tr>
            <td>序号</td>
...
<script>
    function user_add_day(user_id) {
        if (confirm("你确定要增加？")){
            $.ajax({
                url:url+"/admin/gameVirtualUserExpAdd.do",
                type:'POST',
                data:{
                    user_id:user_id,
    ...
    function del(id) {
        if (confirm("你确定要删除此虚拟用户？删除之后不可恢复！")){
            $.ajax({
                url:url+"/admin/delete_virtual_user_game.do",
                type:'POST',
                data:{
                    id:id
                },
