...
<script type="text/javascript">
    ...
    var size = 15;
    var all_url = url+"/admin/show_wechat_platform_challenge.do?page="+page+"&size="+size;
    //--------------------------------------------------------------------
    // 修改虚拟预约用户数(添加输入框)
    //判断是否有输入框
            ...
            success:function (result) {
                ...
                for(var i = 0; i < data.length; i++){
                    $("#info").append('<tr>'+
                        ...
                        '<td style="width: 6%;"><button onclick="single('+"'"+data[i]['id']+"'"+')" style="margin-left: 5px;">查看</button><button style="margin-left: 5px;" onclick="del('+"'"+data[i]['id']+"'"+')">删除</button>(若删除，参加该期的的所有用户记录也会删除)</td>'+
                        '</tr>');
                }
