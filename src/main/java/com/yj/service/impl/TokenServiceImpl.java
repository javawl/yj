...
public class TokenServiceImpl implements ITokenService {
    ...
    protected String wxGameAppID;
    protected String wxGameAppSecret;
    ...
    protected String wxPlatformNormalAccessTokenUrl;
    ...
    TokenServiceImpl(){
        ...
        this.wxGameAppID = WxConfig.wx_game_app_id;
        this.wxGameAppSecret = WxConfig.wx_game_app_secret;
        ...
        this.wxPlatformNormalAccessTokenUrl = WxConfig.wx_platform_normal_access_token_url;
    ...
    public ServerResponse<String> wx_game_token(String portrait, String nickname, String gender, HttpSession session, String code){
        this.code = code;
        String requestUrlParam = String.format("appid=%s&secret=%s&js_code=%s&grant_type=authorization_code", this.wxGameAppID, this.wxGameAppSecret, this.code);
        //发送post请求读取调用微信接口获取openid用户唯一标识
        JSONObject jsonObject = JSON.parseObject( UrlUtil.sendGet( this.wxLoginUrl,requestUrlParam ));
        if (jsonObject.isEmpty()){
            //判断抓取网页是否为空
        ...
        if (jsonObject.isEmpty()){
            ...
            if (loginFail){
                ...
                try {
                    ...
                    e.printStackTrace();
                    logger.error("微信小游戏获取session_key异常",e.getStackTrace());
                    logger.error(e.getMessage(),e);
                    return ServerResponse.createByErrorMessage(e.getMessage());
                }
            }
...
//                        myc.delSession(sess);
    ...
    /*
     * 通过jsonObject生成令牌并存入缓存，并且没有改用户的话注册新用户，有该用户的话即登录
     * @param    微信小游戏接口网页获取的信息jsonObject
     * @return   token
     * */
    private String game_grant_token(JSONObject jsonObject, String portrait, String nickname, String gender, HttpSession session) throws Exception{
        //拿到openID，在数据库里看一下openID存不存在
        //如果存在不处理，不存在数据库里新增一条user
        //生成令牌，准备缓存数据，写入缓存
        //key：令牌
        ...
        String openid = jsonObject.get("openid").toString();
        //查询是否有这个openid
        String id = userMapper.isExistGameOpenid(openid);
        String uid;
        //事务
        DataSourceTransactionManager transactionManager = (DataSourceTransactionManager) ctx.getBean("transactionManager");
        DefaultTransactionDefinition def = new DefaultTransactionDefinition();
        ...
        try {
            if (id == null){
                ...
                //存到数据库
                userMapper.addGameUser(username, portrait, sex, openid, nowStamp);

                //新的user id
                Map<Object,Object> newUser = userMapper.getUserIdByTimeStampGameOpenId(openid, nowStamp);
                //用户id
            ...
            transactionManager.rollback(status);
            e.printStackTrace();
            logger.error("微信小游戏登录异常1",e.getStackTrace());
            logger.error(e.getMessage(),e);
            throw new Exception(e.getMessage());
        }
    }
